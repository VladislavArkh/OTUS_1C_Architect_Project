...
Процедура СтатусПриИзмененииСервер()
	...
	Иначе
		Объект.ДатаРассмотрения = Дата(1,1,1);
	КонецЕсли;
...
Процедура ЧислоДнейПриИзменении(Элемент)
	...
	Элементы.ЧислоДней.Заголовок = Строка(ЧислоДней + " дн.");
...
Процедура ЗаполнитьДанныеОбОплатеЗаявки()
	...
	Если ОстатокОплаты < 0 Тогда
		НадписьФактическаяОплата = СтроковыеФункции.ФорматированнаяСтрока(
			НСтр("ru='<span style=""color: %1"">Не оплачена</span>'"), "ПоясняющийОшибкуТекст");
	ИначеЕсли ОстатокОплаты = 0 Тогда
		Если Объект.Закрыта Тогда
			...
			НадписьФактическаяОплата = СтроковыеФункции.ФорматированнаяСтрока(
				...
				"ПоясняющийОшибкуТекст", ФормированиеПечатныхФорм.ФорматСумм(СуммаЗакрыто, Объект.Валюта));
		Иначе
	...
	Иначе
		НадписьФактическаяОплата = СтроковыеФункции.ФорматированнаяСтрока(
			...
			"ПоясняющийОшибкуТекст", ФормированиеПечатныхФорм.ФорматСумм(ОстатокОплаты, Объект.Валюта));
	КонецЕсли;
...
Функция ОстатокОплатыПоЗаявке(Ссылка)
	...
	Запрос.Текст = "ВЫБРАТЬ
	               ...
	               |////////////////////////////////////////////////////////////////////////////////
	               |ВЫБРАТЬ
	               |	ЗаявкаНаКомандировку.СуммаДокумента КАК НеОплачено
	               |ИЗ
...
Процедура ВывестиИнформациюОБронировании()
	...
	Элементы.СкачатьВсе.Видимость = ЛОЖЬ;
...
Процедура УстановитьСписокСтатусов()
	...
	Если Не ПравоСогласования Тогда
		...
		СписокВыбора.Добавить(Перечисления.СтатусыЗаявокСотрудников.Подготовлено);
		СписокВыбора.Добавить(Перечисления.СтатусыЗаявокСотрудников.Отклонено);
		
		Если ПравоСогласования Или Объект.Статус = Перечисления.СтатусыЗаявокСотрудников.Согласовано Тогда
			СписокВыбора.Добавить(Перечисления.СтатусыЗаявокСотрудников.Согласовано);
		КонецЕсли;
	КонецЕсли;
...
Процедура ИзменитьПараметрЭлементаБронирования(Форма, ИмяЭлемента, ОбратноеНаправление)
	...
	ПараметрыЭлемента = НЕОПРЕДЕЛЕНО;
